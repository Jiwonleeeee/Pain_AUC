...
table(remove_indi)
which(remove_indi==1)
#only one patient -> ID[1487]
# op_general$ID[1488] # ID=2628
...
auc_ftn <- function(df_input,discharge_input,indi_mat_input){
  time_vec <- subset(df_input, select=c("PAIN_RECORDED_TIME"))[,1]
  an_start <- subset(df_input, select=c("AN_START_DATETIME"))[,1]
  an_stop <- subset(df_input, select=c("AN_STOP_DATETIME"))[,1]
  ...
  return(list(auc_save=auc_save,acc_time=acc_time))
}
...
for(t in 1:n){
  ...
  if(t!=1488){ # remove 1488th patient
    ...
    auc_final[t] <- auc_ftn(df_input,discharge_input,indi_mat_input)$auc_save
    duration_final[t] <- auc_ftn(df_input,discharge_input,indi_mat_input)$acc_time
  }
  print(t)
...
# max check
#df_input <- subset(op_pain, ID==op_general$ID[173])

## check > 0 
# patients_check <- subset(op_pain, ID==op_general$ID[6])
# temp_check <- which(check>0)[-1]
# for(i in temp_check){
#   patients_check <- rbind(patients_check, subset(op_pain, ID==op_general$ID[i]))
# }

# write.xlsx(patients_check, sheetName="sheet1", file="patients_needto_check.xlsx")

# remove time vectors under anesthesia

# merge data
temp_data <- data.frame(ID=op_general$ID,Discharge=op_general$Disch_DT, AUC=auc_final,duration=duration_final)

# left_join
op_pain_merge <- left_join(op_pain,temp_data,by="ID")
# remove 1487 (only one ps)
op_pain_merge <- filter(op_pain_merge, ID!=op_general$ID[1488])


# setwd("/Users/wonny/Downloads/CHMC/Pain AUC/Result")
# write.xlsx(op_pain_merge, sheetName="sheet1", file="OP_AUC_combined_data_final.xlsx")
# save(temp_data, file="AUC_op.rda")
