...
auc_OP_merge$percent <- numeric(nrow(auc_OP_merge))
for(t in 1:nrow(auc_OP_merge)){
  if(auc_OP_merge$auc_under4_final[t]==0){
    auc_OP_merge$percent[t] <- 1
  }else{
    auc_OP_merge$percent[t] <- auc_OP_merge$auc_under4_final[t]/auc_OP_merge$AUC[t]
  }
  print(t)
}

auc_OP_merge$percent <- auc_OP_merge$percent * 100
# merge data
