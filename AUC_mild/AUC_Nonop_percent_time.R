...
auc_under4_ftn <- function(df_input, discharge_input){
  ...
  return(list(auc=auc_save,duration=acc_time,pt=percent_time_save))
...
for(t in 1:n){
  ...
  pt_under4_final[t] <- auc_under4_ftn(df_input,discharge_input)$pt
...
for(t in 1:nrow(auc_nonOP_merge)){
  if(auc_nonOP_merge$auc_under4_final[t]==0){
    auc_nonOP_merge$percent_auc[t] <- 1
  }else{
    auc_nonOP_merge$percent_auc[t] <- auc_nonOP_merge$auc_under4_final[t]/auc_nonOP_merge$AUC[t]
  }
  print(t)
}
