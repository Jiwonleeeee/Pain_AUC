...
which(Merge_temp$newID=="ID438")
Merge_temp <- Merge_temp[-440,]

which(Merge_temp$newID=="ID50")
Merge_temp <- Merge_temp[-51,]

which(Merge_temp$newID=="ID845")
Merge_temp <- Merge_temp[-846,]


Merge_data <- subset(Merge_temp, select=-newID)

# IDduplicateSet_flacc <- subset(raw_data, newID=="ID438" | newID=="ID50" | newID=="ID845")
# save(IDduplicateSet_flacc, file="/Users/wonny/Downloads/CHMC/Pain AUC/pod_race_result/diffendtime_flacc.rda")


# apply(Merge_data,2,range)
# export
setwd("/Users/wonny/Downloads/CHMC/AUC pod race/result")
write.xlsx(Merge_data, sheetName="sheet1", file="AUC_pod_flacc.xlsx")
